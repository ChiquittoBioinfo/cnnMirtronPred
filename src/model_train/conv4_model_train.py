...
def train_model_conv4(log_path,model_path):
    conv4_output = model_construction.model_conv4_output\
                                   (input_X,EMBEDDING_SIZE,keep_prob)
    
    #loss_and_optimization(conv6_output,y_train)
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits\
                                (logits = conv4_output,labels = input_y)
    cross_entropy_mean = tf.reduce_mean(cross_entropy)
    # optimization
    train_op = tf.train.AdamOptimizer(LR).minimize(cross_entropy_mean)  
    
    # calculate the accuracy of the model
    correct_prediction = tf.equal(tf.argmax(conv4_output,1), tf.argmax(input_y,1))
    ...
    tp_op,tn_op,fp_op,fn_op = model_evaluate.evaluation_op(conv4_output,input_y)
...
if __name__ == "__main__":
    train_model_conv4("../../logs/filter4","../../logs/filter4/filter4.ckpt")
