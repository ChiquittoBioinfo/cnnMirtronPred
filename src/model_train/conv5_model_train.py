...
def train_model_conv5(log_path,model_path):
    conv5_output = model_construction.model_conv5_output\
                                   (input_X,EMBEDDING_SIZE,keep_prob)
    
    #loss_and_optimization(conv6_output,y_train)
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits\
                                (logits = conv5_output,labels = input_y)
    cross_entropy_mean = tf.reduce_mean(cross_entropy)
    # optimization
    train_op = tf.train.AdamOptimizer(LR).minimize(cross_entropy_mean)  
    
    # calculate the accuracy of the model
    correct_prediction = tf.equal(tf.argmax(conv5_output,1), tf.argmax(input_y,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction,"float"))
    
    tf.summary.scalar("loss",cross_entropy_mean)
    tf.summary.scalar("accuracy",accuracy)

    tp_op,tn_op,fp_op,fn_op = model_evaluate.evaluation_op(conv5_output,input_y)
...
if __name__ == "__main__":
    train_model_conv5("../../logs/filter5","../../logs/filter5/filter5.ckpt")
