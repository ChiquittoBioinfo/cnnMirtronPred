...
def train_model_concat(log_path,model_path):
    concat_output = model_construction.model_concat_output\
                                   (input_X,EMBEDDING_SIZE,keep_prob)
    
    #loss_and_optimization(conv6_output,y_train)
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits\
                                (logits = concat_output,labels = input_y)
    ...
    tp_op,tn_op,fp_op,fn_op = model_evaluate.evaluation_op(concat_output,input_y)
...
if __name__ == "__main__":
    train_model_concat("../../logs/concat","../../logs/concat/concat.ckpt")
